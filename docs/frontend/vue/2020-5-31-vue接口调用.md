...
`Promise.all`方法接受一个数组作参数，数组中的对象（p1、p2、p3）均为 promise 实例（如果不是一个promise，该项会被用`Promise.resolve`转换为一个promise)。它的状态由这三个promise实例决定

#####  .race()

`Promise.race`方法同样接受一个数组作参数。当 p1, p2, p3 中有一个实例的状态发生改变（变为`fulfilled`或`rejected`），p 的状态就跟着改变。并把第一个改变状态的 promise 的返回值，传给p的回调函数
...
基于 `promise` 用于浏览器和 `node.js` 的 `http` 客户端，支持浏览器和 `node.js`，支持 `promise`，能拦截请求和响应，自动转换 `JSON` 数据，能转换请求和响应数据
...
导入`axios` 用来发送 `ajax` ，把获取到的数据渲染到页面上 
...
添加图书之前发送请求验证图示是否已经存在，如果不存在 往后台里面添加图书名称，图书存在与否只需要修改`submitFlag` 的值即可
